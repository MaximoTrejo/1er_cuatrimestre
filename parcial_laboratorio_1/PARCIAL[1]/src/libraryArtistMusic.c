				...
				if(flagPrimerPos==1){
					...
					if(utn_getInt(&opcionInforme,"Opcion", "Error\n", 1, 4, REINTENTOS)==0){
						...
						case 2:
							...
							printf("*Cantidad de álbumes menor al 1/1/2000 es %d\n ",cantFechaM);
							break;
						case 3:
					...
					if(utn_getInt(&opcionListar,"\nOpcion", "Error\n", 1, 15, REINTENTOS)==0){
						...
						case 13:
							...
							if(utn_getInt(&buscarF, "\nIngrese fecha ", "Error", 0, 9999, REINTENTOS)==0){
								...
								if(fechaBuscada >= 0){
									printf("genero\tdescripcion\tid\ttipoArtista\tid\tNOMBRE ARTISTA\t\tID\tTITULO\tDIA\tMES\tAÑO\tIMPORTE\t\ttypeAlbum\tdescripcion\n");
									printAlbum(list[fechaBuscada]);
...
void opcionesInformes(){
    printf("\nMenu de informes"
            ...
            "\n3. (recuperatorio) informar la cantidad de solistas de un año determinado."
    		"\n4.Salir\n");
}
