...
int controller_aumentoTipo(LinkedList *pArrayListPassenger){
	//valido
	int retorno = -1;
	//creo la funcion puntero
	void (*pFunc)(void *this);//es el eComputer
	//valido que haya algo
	if (pArrayListPassenger != NULL){
		//guardo la funcion en el puntero
		pFunc = aumentoTipo;
		//llamo a la funcion map y le paso la lista y el puntero a funcion
		ll_map(pArrayListPassenger, pFunc);
		retorno=0;
	}
	return retorno;
}
