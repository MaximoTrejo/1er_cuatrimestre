...
int filtrarTipoMX1(void* this){
	 ...
	 */
	int retorno = -1;
	char auxCategoria[TAM];//auxiliar donde guardo la categoria
...
int par_compareCategoria(void* p1 ,void*p2){
	eParticipantes* unPasajero;
	eParticipantes*otroPasajero;

	unPasajero=(eParticipantes*)p1;
	otroPasajero=(eParticipantes*)p2;

	return strcmp(unPasajero->categoria,otroPasajero->categoria);
}
