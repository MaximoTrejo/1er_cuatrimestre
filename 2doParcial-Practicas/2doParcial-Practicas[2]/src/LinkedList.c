...
static Node* getNode(LinkedList* this, int nodeIndex)
{
	...
	//nodeIndex porque es el indice al nodo (i)y verifico que no sea mas chico que el largo de la lista
	if(this!=NULL && nodeIndex >=0 && largo>nodeIndex){
		...
		for(int i=0;i<nodeIndex;i++){
			...
			AuxNode=AuxNode->pNextNode;//lo que esta guardando es el gancho al proximo , ejemplo si busco 100 el gancho al siguuente es 101 , pero como el elemento que esta dentro es el del 100 me sirve
		}
	}
...
LinkedList* ll_subList(LinkedList* this,int from,int to)
{
    ...
    if(this!=NULL && from >= 0 && to >= from && to <= this->size){
    	...
    	//recorro el indice de la lista hasta el final de la misma indicada en los parametros (te mandan el inicio y el final de la lista por parametro)
...
LinkedList* ll_map(LinkedList *this, void (*pFunc)(void *element)){
	...
	if (this != NULL && pFunc != NULL) {
		for(int i = 0; i < ll_len(this); i++) {
			aux = ll_get(this, i);//me trae cada elemento
			if(aux != NULL){//si no esta vacio sigo
				//ejecuta la funcion y le pasa el elemento
				pFunc(aux);
...
LinkedList* ll_filter(LinkedList* this, int (*pFunc)(void*)){
    ...
    LinkedList* auxListaFiltrada;//lista aux

    int len = ll_len(this);//saco el largo

    if (this != NULL){//valido
    	auxListaFiltrada = ll_newLinkedList();//creo una nueva lista
        if (auxListaFiltrada != NULL)//valido que haya espacio
        {
            for (int i=0; i<len; i++)//recorro la lista que vino por parametros
            {
                if (pFunc(ll_get(this, i)) != 0)//llamo a la funcion que compara la categoria (le paso el elemento)
                {
                    ll_add(auxListaFiltrada, ll_get(this, i));//lo guardo en la lista aux
                }
            }
        }
    }
    return auxListaFiltrada;//devuelvo la nueva lista con los datos filtrados
}
